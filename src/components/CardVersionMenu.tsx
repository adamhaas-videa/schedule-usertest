...
interface CardVersionMenuProps {
  value: CardVersion;
  onChange: (version: CardVersion) => void;
}

// Mini menu used to demo different patient-card interaction models on the
// schedule. Purely a prototype control for walking through concepts on video.
export default function CardVersionMenu({
  value,
  onChange,
}: CardVersionMenuProps) {
  const active = CARD_VERSIONS.find((v) => v.id === value) ?? CARD_VERSIONS[0];
  ...
  return (
    <Popover>
      <PopoverTrigger className="inline-flex items-center gap-2 h-8 pl-2 pr-2.5 rounded-md border border-border bg-card text-sm font-medium text-foreground hover:bg-muted transition-colors cursor-pointer">
        <i className="fa-regular fa-flask text-muted-foreground text-[13px]" aria-hidden />
        <span className="text-muted-foreground">Card demo</span>
      ...
      <PopoverContent align="end" className="w-80 gap-1 p-1.5">
        <div className="px-2 pt-1.5 pb-1">
          ...
          <div className="text-[11px] text-muted-foreground/80">
            Prototype interaction models — schedule only
          </div>
        </div>
        {CARD_VERSIONS.map((v) => {
