...
import { getNowMinutes, READY_FOR_CHAIR_WINDOW_MIN } from "@/lib/timeline";

/**
 * Returns the mock patients enriched with clock-derived state (status,
 ...
 */
export function getEnrichedPatients(
  nowMinutes: number = getNowMinutes()
): Patient[] {
  return mockPatients.map((p) =>
    applySimulatedTime(p, nowMinutes, READY_FOR_CHAIR_WINDOW_MIN)
  );
}
